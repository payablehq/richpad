...
export function ModernBlockTypeDropdown({
  editor,
}: ModernBlockTypeDropdownProps) {
  ...
  const selectOption = (opt: (typeof blockTypes)[0]) => {
    if (opt.value === "paragraph") editor.chain().focus().setParagraph().run();
      ...
      editor
        .chain()
        .focus()
        .toggleHeading({ level: opt.level as any })
        .run();
    setIsOpen(false);
  };
  ...
  return (
      ...
      trigger={
        <button
          className={cn(
            ...
            "rp-modern-toolbar-btn-text",
            isOpen && "rp-modern-toolbar-btn--active",
          )}
          onClick={() => setIsOpen(!isOpen)}
        >
          <span>
            {currentBlock.value === "heading"
              ? `Heading ${currentBlock.level}`
              : "Normal text"}
          </span>
          <ChevronDown size={14} />
        </button>
    ...
    >
      <div
        className="rp-modern-dropdown-group-title"
        style={{
          padding: "4px 16px",
          fontSize: "11px",
          fontWeight: "bold",
      ...
      {blockTypes.map((opt) => (
        <button
          key={opt.label}
          className={cn(
            "rp-modern-dropdown-item",
            currentBlock.label === opt.label &&
              "rp-modern-dropdown-item--active",
          )}
          onClick={() => selectOption(opt)}
        >
          <span className="rp-modern-dropdown-item-icon">{opt.icon}</span>
          <span
            className="rp-modern-dropdown-item-label"
            style={
              opt.level
                ? { fontSize: `${1.2 - opt.level * 0.05}em`, fontWeight: 600 }
                : {}
            }
          >
            {opt.label}
          </span>
          <span className="rp-modern-dropdown-item-shortcut">
            {opt.shortcut}
          </span>
        </button>
      ))}
    ...
    <DropdownWrapper
      ...
      trigger={
        <button
          className={cn(
            "rp-modern-toolbar-btn",
            "rp-modern-toolbar-split-trigger",
            isOpen && "rp-modern-toolbar-btn--active",
          )}
          onClick={() => setIsOpen(!isOpen)}
        >
    ...
    >
      {formats.map((fmt) => (
        <button
          ...
          className={cn(
            "rp-modern-dropdown-item",
            fmt.isActive && "rp-modern-dropdown-item--active",
          )}
          onClick={() => {
            fmt.action();
            setIsOpen(false);
      ...
      >
        <span className="rp-modern-dropdown-item-icon">
    ...
    >
      {lists.map((list) => (
        <button
          ...
          className={cn(
            "rp-modern-dropdown-item",
            list.isActive && "rp-modern-dropdown-item--active",
          )}
          onClick={() => {
            list.action();
            setIsOpen(false);
...
export function ModernAlignDropdown({ editor }: { editor: Editor }) {
  const [isOpen, setIsOpen] = useState(false);
  ...
  return (
    <DropdownWrapper
      ...
      trigger={
        <button
          className={cn(
            "rp-modern-toolbar-btn",
            "rp-modern-toolbar-align-trigger",
            isOpen && "rp-modern-toolbar-btn--active",
          )}
          onClick={() => setIsOpen(!isOpen)}
    ...
    >
      {alignments.map((align) => (
        ...
        >
          <span className="rp-modern-dropdown-item-icon">{align.icon}</span>
          <span className="rp-modern-dropdown-item-label">{align.label}</span>
          <span className="rp-modern-dropdown-item-shortcut">
            {align.shortcut}
          </span>
        </button>
...
export function ModernColorDropdown({ editor }: { editor: Editor }) {
  ...
  return (
    ...
    >
      <div
        style={{ padding: "8px 16px", fontWeight: "bold", fontSize: "12px" }}
      >
        Text colour
      </div>
